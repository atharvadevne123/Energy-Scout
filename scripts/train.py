    ...
    df = pd.DataFrame(
        {
            ...
            "occupancy_rate": np.where(
                np.isin(dow, [5, 6]), rng.uniform(0, 0.3, n), rng.uniform(0.4, 1.0, n)
            ),
            "day_of_week": dow,
            "hour": hours,
            "month": months,
            ...
            "solar_generation_kw": np.where(
                np.isin(hours, range(8, 19)), rng.uniform(0, 50, n), 0.0
            ),
...
def main():
    ...
    logger.info(
        "Training demand forecaster on {:,} readings with {} features.", len(X), len(feat_cols)
    )
